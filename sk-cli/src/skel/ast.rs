...
pub(super) fn parse_trace_selector(sel: Pair<Rule>, start_ts: i64) -> anyhow::Result<TraceSelector> {
        ...
        Rule::trace_selector_list => {
            ...
            for s in sel.into_inner() {
                match s.as_rule() {
                    ...
                    Rule::ts_conditional => {
                        conditions.push(parse_ts_conditional(s.into_inner(), start_ts));
                    },
                    _ => unreachable!(),
                }
            }
...
pub(super) fn parse_ts_conditional(mut cond: Pairs<Rule>, start_ts: i64) -> Conditional {
    // Unwraps are safe/guaranteed by the grammar
