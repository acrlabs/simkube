...
impl TraceManager {
    pub fn new(client: Client, config: TracerConfig, store: Arc<Mutex<TraceStore>>) -> Self {
    ...
    pub async fn start(&mut self) -> EmptyResult {
        let mut apiset = DynamicApiSet::new(self.client.clone());
        ...
        let pw = pod_watcher::new_with_stream(self.client.clone(), apiset, pod_tx.clone(), self.ready_tx.clone())?;
